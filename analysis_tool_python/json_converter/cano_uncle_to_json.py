...
class ApiToJson:
    ...
    def file_to_json(path, filename, save_path):
        ...
        for line in load_file.load_file_yield_lines(path, filename):
            ...
            data.gasLimit = str(int(load_file.load_field_from_dict(content, "gasLimit", data.gasLimit), 16))
            data.gasUsed = str(int(load_file.load_field_from_dict(content, "gasUsed", data.gasUsed), 16))
            ...
            data.size = str(int(load_file.load_field_from_dict(content, "size", data.size), 16))
